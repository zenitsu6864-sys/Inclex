...
export default function CartDrawer() {
  ...
  return (
    <div className={`fixed inset-0 z-[60] pointer-events-none ${open ? '' : ''}`} aria-hidden={!open}>
      <div
      ...
      >
        <div className="flex items-center justify-between border-b border-black/10 px-6 py-5">
          ...
          <button onClick={() => setOpen(false)} aria-label="Close bag"><X className="h-5 w-5" /></button>
        </div>

        <div className="h-[calc(100%-11rem)] overflow-y-auto px-6 py-6">
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <div className="grid h-16 w-16 place-items-center rounded-full border border-black/10 bg-white text-[#C9A227]"><ShoppingBag className="h-6 w-6" /></div>
              <p className="mt-5 font-serif text-2xl">Your bag is empty</p>
              <p className="mt-2 max-w-xs text-sm text-neutral-500">Discover our premium keychains crafted for those who carry more than keys.</p>
              <Link href="/shop" onClick={() => setOpen(false)} className="btn-dark mt-8">Shop Collection <ArrowRight className="h-4 w-4" /></Link>
            </div>
          ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-0.5 text-xs text-neutral-500">{it.subtitle}{it.color ? ` • ${it.color}` : ''}</div>
                        {it.engraving && (<div className="mt-1 text-xs italic text-[#C9A227]">Engraving: “{it.engraving}”</div>)}
                      </div>
                      <button onClick={() => remove(it.key)} aria-label="Remove" className="text-neutral-400 hover:text-black"><Trash2 className="h-4 w-4" /></button>
                    </div>
                    <div className="mt-3 flex items-center justify-between">
                      <div className="inline-flex items-center rounded-sm border border-black/10 bg-white">
                        <button onClick={() => setQty(it.key, it.qty - 1)} className="px-3 py-1.5"><Minus className="h-3 w-3" /></button>
                        <span className="px-3 text-sm font-medium">{it.qty}</span>
                        <button onClick={() => setQty(it.key, it.qty + 1)} className="px-3 py-1.5"><Plus className="h-3 w-3" /></button>
                      </div>
        ...
        <div className="absolute inset-x-0 bottom-0 border-t border-black/10 bg-white px-6 py-5">
          ...
          <Link href="/checkout" onClick={() => setOpen(false)} className={`btn-dark mt-4 w-full ${items.length === 0 ? 'pointer-events-none opacity-40' : ''}`}>Checkout <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </aside>
    </div>
  );
