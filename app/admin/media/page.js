...
import { UploadCloud, Trash2, Copy, ImageIcon, Loader2 } from "lucide-react";
    ...
    <AdminShell title="Media Library" subtitle={`${media.length} items`}>
      <div className="mb-6 rounded-sm border border-black/10 bg-white p-6">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <label className="w-56">
            ...
            <input
              ...
              className="mt-2 w-full rounded-sm border border-black/10 bg-white px-4 py-2.5 text-sm"
            />
          </label>
          <button
            onClick={selectedFile ? uploadFile : add}
            disabled={uploading}
        ...
        <p className="mt-3 text-xs text-neutral-500">
          Upload images or videos directly from your computer, or leave the file
          empty and paste a public URL.
        </p>
      </div>
      ...
      {!loading && media.length === 0 && (
        <div className="grid place-items-center rounded-sm border border-dashed border-black/10 bg-white p-16 text-center">
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Add your first image or video URL above.
          </p>
        </div>
      )}
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {media.map((m) => (
          <div
            key={m.id}
            className="group overflow-hidden rounded-sm border border-black/10 bg-white"
          >
