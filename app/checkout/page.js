...
export default function CheckoutPage() {
  ...
  const freeShippingAbove = Number(settings?.shipping?.freeAbove ?? 299);
const flatShipping = Number(settings?.shipping?.flat ?? 49);

const shipping =
  subtotal === 0 || subtotal >= freeShippingAbove
    ? 0
    : flatShipping;
...
const total = subtotal + shipping;
    ...
    return (
      <main className="min-h-screen bg-[#F8F7F4]">
        ...
        <section className="container-editorial grid place-items-center py-32 text-center">
          <div className="grid h-20 w-20 place-items-center rounded-full bg-[#C9A227]/15 text-[#C9A227]">
            <Check className="h-9 w-9" />
          </div>
          <h1 className="mt-8 font-serif text-5xl">Thank you.</h1>
          <p className="mt-3 max-w-md text-neutral-600">
            Your order <b className="text-black">{order.orderNumber}</b> has
            been placed. A confirmation has been sent to your email with the
            tracking link.
          </p>
          <div className="mt-10 flex gap-3">
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 rounded-sm border border-black/10 bg-white px-6 py-3.5 text-sm font-semibold"
            >
              Back to Home
            </Link>
          </div>
  ...
  Free shipping on orders above ₹{fmt(freeShippingAbove)} • Secure checkout
</p>
          ...
          <div className="col-span-12 lg:col-span-7 space-y-8">
            {!user && (
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-sm border border-black/10 bg-white p-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-sm bg-[#F8F7F4] text-[#C9A227]">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-neutral-500">
                      Sign in for faster checkout and to save this order to your
                      profile.
                    </div>
                  </div>
                </div>
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-sm border border-black/10 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] hover:border-black"
                >
            ...
            {user && (
              <div className="flex items-center gap-3 rounded-sm border border-emerald-100 bg-emerald-50/50 p-4">
                <span className="grid h-9 w-9 place-items-center rounded-sm bg-emerald-100 text-emerald-700">
                  <Check className="h-4 w-4" />
                </span>
                <div className="text-sm">
                  Signed in as <b>{user.name}</b> ({user.email}) — this order
            ...
            <Panel title="Payment" number="03">
              <div className="grid gap-2">
                {[
                  {
                    id: "razorpay",
                    label: "UPI / Cards / Netbanking / Wallet",
                    hint: "Secure payment via Razorpay — instant confirmation",
                  },
                  {
                    id: "cod",
                  ...
                  <label
                    key={p.id}
                    className={`flex items-center justify-between rounded-sm border px-4 py-3 cursor-pointer transition ${payment === p.id ? "border-black bg-black text-white" : "border-black/10 bg-white hover:border-black"}`}
                  >
                    <div>
            ...
            <div className="sticky top-28 rounded-sm border border-black/10 bg-white p-6">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 text-[11px]">
                <div className="flex items-start gap-2">
                 ...
                 <span>Free shipping above ₹{fmt(freeShippingAbove)}</span>
                </div>
                <div className="flex items-start gap-2">
...
function Panel({ number, title, children }) {
  return (
    <div className="rounded-sm border border-black/10 bg-white p-6 md:p-8">
      <div className="flex items-center gap-3">
        <span className="text-xs font-semibold tracking-[0.24em] text-[#C9A227]">
          {number}
        </span>
