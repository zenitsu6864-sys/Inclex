...
export default function SignupClient(){
  ...
  useEffect(() => {
    async function loadBanner() {
      try {
        ...
        setHeroImage(
          data.homepage?.signupBanner || "/uploads/images/signup-banner.jpg",
        );
      } catch (err) {
        console.error(err);
        setHeroImage("/uploads/images/signup-banner.jpg");
      }
    }
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (r.ok && data.ok) {
        toast.success("Welcome to Inclex", {
          description: `Hi ${data.user.name} — your studio is ready.`,
        });
        await refresh();
      ...
      <section className="container-editorial grid min-h-[calc(100vh-9rem)] grid-cols-1 items-center gap-14 py-12 lg:grid-cols-2 lg:py-20">
        <div className="mx-auto w-full max-w-md order-2 lg:order-1">
          ...
          <p className="mt-2 text-neutral-500">
            Save designs, track orders, and unlock member offers.
          </p>
            ...
            <label className="block">
              ...
              <div className="relative mt-2">
                ...
                <input
                  ...
                  className="w-full rounded-sm border border-black/10 bg-white pl-10 pr-4 py-3 focus:border-[#C9A227] focus:outline-none focus:ring-2 focus:ring-[#C9A227]/25"
                />
              </div>
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              <div className="relative mt-2">
                ...
                <input
                  ...
                  className="w-full rounded-sm border border-black/10 bg-white pl-10 pr-10 py-3 focus:border-[#C9A227] focus:outline-none focus:ring-2 focus:ring-[#C9A227]/25"
                />
                <button
                  type="button"
                  onClick={() => setShowPw((v) => !v)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-sm p-1.5 text-neutral-400 hover:text-black"
                >
                  {showPw ? (
              ...
              <div className="mt-2 flex gap-1">
                {[1, 2, 3].map((i) => (
                  <span
                    key={i}
                    className={`h-1 flex-1 rounded-full ${strength >= i ? (strength === 1 ? "bg-red-400" : strength === 2 ? "bg-amber-400" : "bg-emerald-500") : "bg-black/10"}`}
                  />
                ))}
              </div>
            </label>
          ...
          <p className="mt-6 text-center text-sm text-neutral-600">
            ...
            <Link
              ...
              className="font-semibold text-black hover:text-[#C9A227]"
            >
              Sign in
            </Link>
          </p>
          ...
          <div className="relative z-10 flex h-full flex-col justify-between p-10 text-white">
            ...
            <div>
              <ul className="space-y-4">
                {[
                  "Save your bespoke designs",
                  "Faster checkout & tracked orders",
                  "Early access to new collections",
                  "Exclusive member offers",
                ].map((p) => (
                  <li key={p} className="flex items-center gap-3">
                    <span className="grid h-6 w-6 place-items-center rounded-full bg-[#C9A227] text-black">
