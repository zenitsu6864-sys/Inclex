    ...
    return (
      <div className="grid place-items-center rounded-sm border border-dashed border-black/10 bg-white p-16 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-neutral-500">
          Once you place your first order, it’ll show up here — with live
          tracking updates.
        </p>
      ...
      {orders.map((o) => (
        <div key={o.id} className="rounded-sm border border-black/10 bg-white">
          ...
          <div className="flex items-center justify-between border-t border-black/[0.06] p-4">
            ...
            <Link
              href="/contact"
              className="text-xs uppercase tracking-[0.16em] text-neutral-600 hover:text-black"
            >
...
function StatusPill({ status }) {
  ...
  return (
    <span
      className={`inline-flex rounded-sm px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${map[status] || "bg-neutral-100 text-neutral-700"}`}
    >
      {status}
    </span>
  );
